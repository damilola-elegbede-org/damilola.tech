...
All tokens are age-encrypted at rest under `<agent>/.credentials/` and `infra/credentials/shared/`, decrypted per-invocation. GitHub App tokens are minted fresh on every git or `gh` call — no long-lived token ever touches memory or logs.
...
├── launchd (82 active plists, bareclaude.* namespace)
